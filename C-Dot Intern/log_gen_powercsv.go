...
func generateData(thresholds Thresholds) PowerPlantData {
	...
	if isAnomaly {
		...
		temp = randomRange(
			thresholds.MaxBoilerTemp+10,
			thresholds.MaxBoilerTemp+80,
		)

		pressure = randomRange(
			thresholds.MaxPressure+1,
			thresholds.MaxPressure+8,
		)

		rpm = rand.Intn(2000) + thresholds.MaxRPM + 1

		vibration = randomRange(
			thresholds.MaxVibration+1,
			thresholds.MaxVibration+10,
		)

		power = randomRange(
			thresholds.MaxPowerMW+50,
			thresholds.MaxPowerMW+300,
		)
...
func main() {
	...
	thresholds := Thresholds{
		MaxBoilerTemp: askFloat(
			reader,
			"Max Boiler Temperature (°C)",
			600,
		),

		MaxPressure: askFloat(
			reader,
			"Max Steam Pressure (MPa)",
			25,
		),
	...
	outputFile, err := os.Create("powerplant_logs.csv")
	if err != nil {
		panic(err)
	}
